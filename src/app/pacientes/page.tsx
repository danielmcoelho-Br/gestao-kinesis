...
export default function PacientesPage() {
  ...
  if (!data) return <div>Nenhum dado encontrado. Faça upload do perfil dos pacientes.</div>;
  ...
  const mapCenter = kinesisLocation;
  const heatmapPoints = stats.heatmapData.map((p: any) => new google.maps.LatLng(p.lat, p.lng));
      ...
      {activeView === 'stats' && (
        <div className="fade-in">
          ...
          <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px', marginBottom: '32px' }}>
            <MetricCard title="Pacientes Atendidos" value={stats.totalAttended} icon={<Users />} color="var(--primary)" />
            <MetricCard title="Perfis Localizados" value={stats.withProfile} icon={<UserCircle />} color="#10b981" />
            <MetricCard title="Aguardando Cadastro" value={stats.missingProfile} icon={<Filter />} color="#f59e0b" />
          </section>

          {/* Apresentação Principal: Gênero e Idade */}
          <div className="card" style={{ marginBottom: '32px' }}>
            <h3 style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <Users size={20} color="var(--primary)" /> Listagem de clientes por Gênero e Idade
            </h3>
                  ...
                  {stats.stratifiedAgeData.map((row: any, i: number) => {
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(0,0,0,0.02)' }}>
                        ...
                        <td style={{ padding: '12px', textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: '600', minWidth: '120px' }}>
                          {row.label}
                        </td>
                        <td style={{ padding: '12px' }}>
      ...
      {activeView === 'list' && (
        <div className="fade-in card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            ...
            <button className="btn" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Download size={18} /> Exportar Lista</button>
          </div>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table className="data-table">
              ...
              <tbody>
                ...
                {missingProfiles.length > 0 && searchTerm === "" && (
                  <tr style={{ background: 'rgba(245, 158, 11, 0.05)' }}>
                    <td colSpan={6} style={{ textAlign: 'center', padding: '20px', color: '#f59e0b', fontWeight: '600' }}>
                      ⚠️ Existem {missingProfiles.length} pacientes atendidos sem perfil cadastrado.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="fade-in card" style={{ height: '75vh', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: 'var(--text-secondary)', padding: '0 10px' }}>
                <span style={{ width: '12px', height: '12px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.6)' }}></span> Alta Densidade
              </div>
            </div>
          </div>
          
          <div style={{ flex: 1, background: '#f8fafc', borderRadius: '16px', border: '1px solid var(--border-color)', position: 'relative', overflow: 'hidden' }}>
            {isLoaded ? (
              <GoogleMap
              ...
              <div style={{ textAlign: 'center', padding: '40px' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '24px', maxWidth: '400px', margin: '0 auto 24px' }}>
                  Para visualizar o mapa, adicione sua chave da API do Google no arquivo <strong>.env</strong> como <strong>NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</strong>.
                </p>
                <div style={{ padding: '20px', background: 'white', borderRadius: '12px', border: '1px solid var(--border-color)', textAlign: 'left', maxWidth: '400px', margin: '0 auto' }}>
                  <div style={{ fontWeight: 'bold', marginBottom: '8px', fontSize: '0.8rem', textTransform: 'uppercase' }}>Status Geográfico:</div>
                  <div style={{ fontSize: '0.85rem' }}>• {stats.heatmapData.length} coordenadas processadas</div>
                  <div style={{ fontSize: '0.85rem' }}>• {stats.withProfile - stats.heatmapData.length} endereços pendentes</div>
                </div>
              </div>
            )}
          </div>
